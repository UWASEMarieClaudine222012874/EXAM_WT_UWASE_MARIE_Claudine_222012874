...
<h1> Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="leader_id" name="leader_id"><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO groupleaders (leader_id,leader_name) VALUES (?, ?)");
        $stmt->bind_param("ss", $leader_id,$leader_name);

        // Set parameters from POST data with validation (optional)
        $leader_id = intval($_POST['leader_id']); // Ensure integer for ID
        $leader_name = htmlspecialchars($_POST['leader_name']); // Prevent XSS
...
$sql = "SELECT * FROM groupleaders";
$result = $connection->query($sql);

?>
<h2>Data for group leaders Form</h2>
    <table border="1">
        <tr>
            <th>leader ID</th>
            <th>leader text</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $leader_id = $row['leader_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['leader_id'] . "</td>
                    <td>" . $row['leader_name'] . "</td>
                    <td><a style='padding:2px' href='delete_groupleaders.php?leader_id=$leader_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_groupleaders.php?leader_id=$leader_id'>Update</a></td> 
                </tr>";
            }
        } else {
