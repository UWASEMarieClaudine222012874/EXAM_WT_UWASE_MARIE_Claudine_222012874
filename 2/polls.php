...
<h1>message Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="poll_id" name="poll_id"><br><br>

        <label for="poll_question">poll_question:</label>
        <input type="text" id="poll_question" name="poll_question" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO polls (poll_id,poll_question,session_id) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $poll_id,$poll_question,$session_id);

        // Set parameters from POST data with validation (optional)
        $poll_id = intval($_POST['poll_id']); // Ensure integer for ID
        $poll_question = htmlspecialchars($_POST['poll_question']); // Prevent XSS
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $poll_id = $row['poll_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['poll_id'] . "</td>
                    <td>" . $row['poll_question'] . "</td>
                    <td>" . $row['session_id'] . "</td>
                    <td><a style='padding:2px' href='delete_polls.php?poll_id=$poll_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_polls.php?poll_id=$poll_id'>Update</a></td> 
                </tr>";
            }
        } else {
