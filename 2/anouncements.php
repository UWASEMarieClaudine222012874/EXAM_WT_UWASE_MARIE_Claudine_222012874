...
<h1> Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="announcement_id" name="announcement_id"><br><br>

        <label for="announcement_text">announcement_text:</label>
        <input type="text" id="announcement_text" name="announcement_text" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO announcements (announcement_id,announcement_text,group_leader_id) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $announcement_id,$announcement_text,$group_leader_id);

        // Set parameters from POST data with validation (optional)
        $announcement_id = intval($_POST['announcement_id']); // Ensure integer for ID
        $announcement_text = htmlspecialchars($_POST['announcement_text']); // Prevent XSS
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $announcement_id = $row['announcement_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['announcement_id'] . "</td>
                    <td>" . $row['announcement_text'] . "</td>
                    <td>" . $row['group_leader_id'] . "</td>
                    <td><a style='padding:2px' href='delete_anouncements.php?announcement_id=$announcement_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_anouncements.php?announcement_id=$announcement_id'>Update</a></td> 
                </tr>";
            }
        } else {
