...
<h1> Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO attendance (attendance_id,session_id,member_id) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $attendance_id,$session_id,$member_id);

        // Set parameters from POST data with validation (optional)
        $attendance_id = intval($_POST['attendance_id']); // Ensure integer for ID
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $attendance_id = $row['attendance_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['attendance_id'] . "</td>
                    <td>" . $row['session_id'] . "</td>
                    <td>" . $row['member_id'] . "</td>
                    <td><a style='padding:2px' href='delete_attendance.php?attendance_id=$attendance_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_attendance.php?attendance_id=$attendance_id'>Update</a></td> 
                </tr>";
            }
        } else {
