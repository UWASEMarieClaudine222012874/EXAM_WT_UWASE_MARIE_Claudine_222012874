...
<head>
  ...
  <title>session group sessions table</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<h1> Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO groupsessions (session_id,session_date,group_leader_id) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $session_id,$session_date,$group_leader_id);

        // Set parameters from POST data with validation (optional)
        $session_id = intval($_POST['session_id']); // Ensure integer for ID
        $session_date = htmlspecialchars($_POST['session_date']); // Prevent XSS
...
$sql = "SELECT * FROM groupsessions";
$result = $connection->query($sql);

?>
<h2>Data for groupsessions Form</h2>
    <table border="1">
        <tr>
            <th>session ID</th>
            <th>session Date</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $session_id = $row['session_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['session_id'] . "</td>
                    <td>" . $row['session_date'] . "</td>
                    <td>" . $row['group_leader_id'] . "</td>
                    <td><a style='padding:2px' href='delete_groupsessions.php?session_id=$session_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_groupsessions.php?session_id=$session_id'>Update</a></td> 
                </tr>";
            }
        } else {
