...
<h1>message Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="message_id" name="message_id"><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO messages (message_id,message_text,member_id) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $message_id,$message_text,$member_id);

        // Set parameters from POST data with validation (optional)
        $message_id = intval($_POST['message_id']); // Ensure integer for ID
        $message_text = htmlspecialchars($_POST['message_text']); // Prevent XSS
        $member_id = htmlspecialchars($_POST['member_id']); // Prevent XSS
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $message_id = $row['message_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['message_id'] . "</td>
                    <td>" . $row['message_text'] . "</td>
                    <td>" . $row['member_id'] . "</td>
                    <td><a style='padding:2px' href='delete_messages.php?message_id=$message_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_messages.php?message_id=$message_id'>Update</a></td> 
                </tr>";
            }
        } else {
