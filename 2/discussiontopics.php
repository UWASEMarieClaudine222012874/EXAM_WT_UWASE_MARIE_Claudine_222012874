...
<h1> Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="text" id="topic_text" name="topic_text" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO discussiontopics (topic_id,topic_text,session_id) VALUES (?, ?, ?)");
        $stmt->bind_param("sis", $topic_id,$topic_text,$session_id);

        // Set parameters from POST data with validation (optional)
        $topic_id = intval($_POST['topic_id']); // Ensure integer for ID
        $topic_text = htmlspecialchars($_POST['topic_text']); // Prevent XSS
...
$sql = "SELECT * FROM discussiontopics";
$result = $connection->query($sql);

?>
<h2>Data for discussiontopics Form</h2>
    <table border="1">
        <tr>
            <th>topic ID</th>
            <th>topic text</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $topic_id = $row['topic_id']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['topic_id'] . "</td>
                    <td>" . $row['topic_text'] . "</td>
                    <td>" . $row['session_id'] . "</td>
                    <td><a style='padding:2px' href='delete_discussiontopics.php?topic_id=$topic_id'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_discussiontopics.php?topic_id=$topic_id'>Update</a></td> 
                </tr>";
            }
        } else {
